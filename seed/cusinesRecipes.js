const db = require('../db')
const { Cusine, Recipe } = require('../models')

db.on('error', console.error.bind(console, 'MongoDB connection error:'))
cusineArray = ['Italian', 'Asian', 'American', 'Latin', 'Dessert', 'French']

const main = async () => {
  const italian = await new Cusine({
    title: 'Italian'
  })
  italian.save()

  const asian = await new Cusine({
    title: 'Asian'
  })
  asian.save()

  const american = await new Cusine({
    title: 'American'
  })
  american.save()
